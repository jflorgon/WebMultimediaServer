...
const SCROLL_KEY = 'series-scroll-y'

export function SeriesPage() {
  const { t } = useTranslation()
  const { items, totalCount, loading, title, genre, fetchAll, appendItems, setTitle, setGenre } = useSeriesStore()
  ...
  const availableGenres = useMemo(
    () => Array.from(new Set(items.flatMap((s) => s.genres ?? []))).sort(),
    [items]
  )
    ...
    >
      <div style={{ paddingTop: '4rem', paddingBottom: '1rem', paddingLeft: '4vw', paddingRight: '4vw' }}>
        <div className="flex items-baseline gap-4" style={{ marginBottom: '2rem' }}>
          <h1 className="text-3xl font-black text-white">{t('nav.series')}</h1>
